#' @title age_of_vehicle_stolen
#'
#' @description Builds the "Age of vehicle stolen" table for a headline crime type, as seen in the "Nature of Crime" dataset
#' for that headline crime type. Currently, the only headline crime type this is built for is "Vehicle-related theft".
#'
#' @details This function builds an "Age of vehicle stolen" table based on two configuration files. The first config
...
#' one corresponding to each part of the table. For the "Age of vehicle stolen" table, there is currently only 1 part, which is built
#' by the function \code{age_of_vehicle_stolen_age}, which returns a data frame.
#'
...
#' configuration file.
#'
#' @return The "Age of vehicle stolen" table for the headline crime type specified by \code{config}, as a \code{GPTable} object.
#'
#' @examples config = autotable::process_config(yaml::read.yaml("configs/age_of_vehicle_stolen/VEHICLE_RELATED_THEFT.yaml"))
#' master_config = yaml::read_yaml(file = "configs/MASTER.yaml")
#' age_of_vehicle_stolen(config = config, master_config = master_config)
#'
...
age_of_vehicle_stolen = function(config, master_config, return_as_dataframes = FALSE) {
...
for (i in c(1:length(subtables_to_build))) {
  ...
  futile.logger::flog.info("age_of_vehicle_stolen - Currently building the subtable '%s'", subtables_to_build[[i]])

   # 2. Call the relevant part-function to build the subtable currently under consideration:

   AoVS = autotable::age_of_vehicle_stolen_age(config = config, master_config = master_config, subtable = i)
   ...
   subtable             = cbind(' ', rownames(AoVS), AoVS, stringsAsFactors = FALSE)
   ...
   # 5. Check whether any bases are below the caution threshold, and - where they are - mark the column heading with a footnote placeholder.

   for (j in c(1:ncol(subtable))) {

      if ((is.numeric(subtable[nrow(subtable), j]) && !is.factor(subtable[nrow(subtable), ]) && subtable[nrow(subtable), j] < master_config$UNWEIGHTED_BASES$CAUTION_THRESHOLD)) {
   ...
   futile.logger::flog.info("age_of_vehicle_stolen - Finished building the subtable '%s'", subtables_to_build[[i]])
...
futile.logger::flog.trace("age_of_vehicle_stolen - Converting the subtables to GPTable objects")

for (i in c(1:length(subtables_as_GPTable_objects))) {

  # 9. Set up the necessary parameters for the GPTable() function:
  table         = subtables_built[[i]]
  title         = config$TITLES_OF_SUBTABLES[i]
  subtitles     = config$METADATA$SUBTITLES
  units         = " "
  ...
  subtable_as_a_GPTable_object = gptables$GPTable(table                 = table,
                                                  title                 = title,
                                                  subtitles             = subtitles,
                                                  units                 = units,
                                                  scope                 = scope,
...
futile.logger::flog.trace("age_of_vehicle_stolen - subtables were successfully converted to GPTable objects")
