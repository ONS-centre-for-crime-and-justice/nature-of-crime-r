#'@title where_incident_occurred
#'
#'@description Builds the "Where incidents occurred" Table for a headline crime type, as seen in the "Nature of Crime"
#'dataset for that headline crime type. See the following URL for an example of a Nature of Crime dataset, for the headline
#'crime type "Robbery":
#'\url{https://www.ons.gov.uk/peoplepopulationandcommunity/crimeandjustice/datasets/natureofrobbery}.
#'
#'@details This function builds an "Point of entry in incidents" Table based on a configuration file, where this
#'configuration file corresponds to one of the headline crime types. The table is processed
#'where the processing is handled by \code{build_where_incident_occurred} By
...
#'
#'@examples where_incident_occurred(config = "configs/where_incident_occurred/ROBBERY.yaml", master_config = master_config)
#'
#'@export
#'

where_incident_occurred = function(config, master_config, return_as_dataframes = FALSE) {
   ...
   for (i in c(1:length(subtables_to_build))) {
      ...
      futile.logger::flog.info("where_incident_occurred - Currently building the subtable '%s'", subtables_to_build[[i]])
      
      # 2. Call the relevant build_ functions to build both parts of the subtable currently under consideration:
      
      WIO   = autotable::build_where_incident_occurred(config = config, master_config = master_config, subtable = i)
      ...
      WIO               = cbind(' ', rownames(WIO), WIO, stringsAsFactors = FALSE)
      names(WIO)[1:2]   = c("index1", "index2")
      ...
      futile.logger::flog.info("where_incident_occurred - Finished building the subtable '%s'", subtables_to_build[[i]])
   ...
   futile.logger::flog.trace("where_incident_occurred - Converting the subtables to GPTable objects")
   ...
   for (i in c(1:length(subtables_as_GPTable_objects))) {
      ...
      annotations = as.list(config$TABLE$FOOTNOTES)
   ...
   futile.logger::flog.trace("where_incident_occurred - subtables were successfully converted to GPTable objects")
