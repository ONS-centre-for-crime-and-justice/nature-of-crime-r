#'@title point_of_entry
#'
#'@description Builds the "Point of entry of incidents" Table for a headline crime type, as seen in the "Nature of Crime"
...
#'where the bulk of this processing is handled by \code{build_point_of_entry} and
#'\code{build_type_of_emotional_response_experienced}, which build the two halves/parts of the subtable respectively. By
...
#'
#'@examples point_of_entry(config = "configs/point_of_entry/BURGLARY.yaml", master_config = master_config)
#'
#'@export
#'

point_of_entry = function(config, master_config, return_as_dataframes = FALSE) {
   ...
   for (i in c(1:length(subtables_to_build))) {
      ...
      futile.logger::flog.info("point_of_entry - Currently building the subtable '%s'", subtables_to_build[[i]])

      # 2. Call the relevant build_ functions to build both parts of the subtable currently under consideration:

      POE   = autotable::build_point_of_entry(config = config, master_config = master_config, subtable = i)
      ...
      POE               = cbind(' ', rownames(POE), POE, stringsAsFactors = FALSE)
      names(POE)[1:2]   = c("index1", "index2")
      ...
      futile.logger::flog.info("point_of_entry - Finished building the subtable '%s'", subtables_to_build[[i]])
   ...
   futile.logger::flog.trace("point_of_entry - Converting the subtables to GPTable objects")
   ...
   futile.logger::flog.trace("point_of_entry - subtables were successfully converted to GPTable objects")
