#'@title Contact with Offenders
#'
#'@description Builds the "COntact with Offenders" Table for a headline crime type, as seen in the "Nature of Crime"
#'dataset for that headline crime type. 
#'
#'@details This function builds an "Contact with Damage" Table based on a configuration file, where this
...
#'\code{contact_with_offenders}, which build the two halves/parts of the subtable respectively. By
...
#'
#'@examples contact_with_offenders(config = "configs/contact_with_offenders/BURGLARY.yaml", master_config = master_config")
#'
#'@export
#'

contact_with_offenders = function(config, master_config, return_as_dataframes = FALSE) {
  ...
  for (i in c(1:length(Subtables_to_build))) {
    ...
    # part of the table needs to be built or not. The grepl uses the "non-connected" part of the subtable title to determine this. 
        ...
        if (grepl("non-connected", Subtables_to_build[[i]], ignore.case = TRUE)) {
          OC                   = offender_contact_build_offender_contact(config = config, master_config = master_config, subtable = i)
          OC                   = cbind(' ', rownames(OC), OC, stringsAsFactors = FALSE)
          names(OC)[1:2]       = c("index1", "index2")
          Subtable             = OC
          rownames(Subtable)   = c()
          ...
          OC                   = offender_contact_build_offender_contact(config = config, master_config = master_config, subtable = i)
          OV                   = offender_contact_build_offender_violence(config = config, master_config = master_config, subtable = i)
          OC                   = cbind(' ', rownames(OC), OC, stringsAsFactors = FALSE)
          OV                   = cbind(' ', rownames(OV), OV, stringsAsFactors = FALSE)
          names(OC)[1:2]       = c("index1", "index2")
          names(OV)[1:2]       = c("index1", "index2")
          OV[1, 1]             = config$PART_TWO$NAME
          Subtable             = rbind(OC, OV)
          rownames(Subtable)   = c()
          Subtables_built[[i]] = Subtable
        }
