#'@title perceived_seriousness
#'
#'@description Builds the "Perceived seriousness of incidents" Table for a headline crime type, as seen in the "Nature of Crime"
...
#'
#'@details This function builds an "Perceived seriousness of incidents" Table based on a configuration file, where this
#'configuration file corresponds to one of the headline crime types. Each subtable of the Table is processed in turn,
#'where the bulk of this processing is handled by \code{build_percetange_perceived_crime} and
#'\code{build_rated_seriousness}, which build the two halves/parts of the subtable respectively. By
...
#'
#'@examples perceived_seriousness(config = "configs/perceived_seriousness/BURGLARY.yaml", master_config = master_config)
#'
#'@export
#'

perceived_seriousness = function(config, master_config, return_as_dataframes = FALSE) {
   ...
   for (i in c(1:length(subtables_to_build))) {
      ...
      futile.logger::flog.info("perceived_seriousness - Currently building the subtable '%s'", subtables_to_build[[i]])
      
      # 2. Call the relevant build_ functions to build both parts of the subtable currently under consideration:
      
      PPC   = autotable::build_percentage_perceived_crime(config = config, master_config = master_config, subtable = i)
      RS = autotable::build_rated_seriousness(config = config, master_config = master_config,
                                                                      subtable = i)
      ...
      PPC               = cbind(' ', rownames(PPC), PPC, stringsAsFactors = FALSE)
      RS             = cbind(' ', rownames(RS), RS, stringsAsFactors = FALSE)
      names(PPC)[1:2]   = c("index1", "index2")
      names(RS)[1:2] = c("index1", "index2")
      RS[1, 1]       = config$PART_ONE$NAME
      
      # 4. Bind the two parts of the subtable together, and blank the dataframe's rownames so that the gptables indexing will
      #    work properly:
      
      subtable = rbind(PPC, RS)
      rownames(subtable) = c()
      
      # 5. Store the finished subtable in our list of subtables built:
      subtables_built[[i]] = subtable
      
      futile.logger::flog.info("perceived_seriousness - Finished building the subtable '%s'", subtables_to_build[[i]])
   ...
   futile.logger::flog.trace("perceived_seriousness - Converting the subtables to GPTable objects")
   ...
   futile.logger::flog.trace("perceived_seriousness - subtables were successfully converted to GPTable objects")
