#'@title number_of_incidents_build_offence_percentages
#'
#'@description Builds the "Proportions by Offence Type" (BOT) part/half of one subtable of an "Number of Incidents against men and women"
...
#'
#'@examples number_of_incidents_build_offence_percentages(config = "configs/number_of_incidents_against_men_and_women/BURGLARY.yaml", master config = master config", subtable = 1)
#'
#'@export
...
number_of_incidents_build_offence_percentages <- function(config, master_config, subtable) {
  ...
  futile.logger::flog.info("build_offence_percentages - Reading in source data from %s",
  ...
  futile.logger::flog.info("build_offence_percentages - Using %s as the weight variable", Weight_variable)
  ...
  futile.logger::flog.info("build_offence_percentages - Removed %s cases out of %s because their weights were 0",
                           nrow(NVF_dataset) - nrow(BOT_1), nrow(NVF_dataset))

  # 6. Extract the unweighted base:

  Unweighted_base = nrow(BOT_1)
  futile.logger::flog.info("build_offence_percentages - Recoded variables and determined the unweighted base as %s",
                           Unweighted_base)

  # 7. Weight the data

  Variables_for_rows = unlist(lapply(config$PART_ONE$ROW_LABELS, `[[`, 1))
  BOT_2 = autotable::apply_weight(BOT_1, base::c(Variables_for_rows), weight = Weight_variable)
  futile.logger::flog.info("build_offence_percentages - Weighted the data using %s", Weight_variable)
  ...
  for (i in c(1:length(Variables_for_rows))) {
    ...
    futile.logger::flog.info("build_offence_percentages - Starting the calculations for %s", offence)
    ...
    futile.logger::flog.info("build_offence_percentages - Calculated the number of male incidents")
    ...
    futile.logger::flog.info("build_offence_percentages - Calculated the number of female incidents")

    # Calculate Total Means and Totals
    all_incidents   = as.data.frame(sum(BOT_2[, offence]) / 1000000)
    futile.logger::flog.info("build_offence_percentages - Calculated the number of total incidents")

    # Bind together
    BOT_5 = as.data.frame(cbind(male_incidents, female_incidents, all_incidents))
    futile.logger::flog.info("build_offence_percentages - Binded the columns together")

    # Name the rows and columns
    column_names = c("  Men", "  Women", "  All")
    rownames(BOT_5) <- offence
    colnames(BOT_5) <- column_names
    futile.logger::flog.info("build_offence_percentages - Named the columns and rows for the variable")
    ...
    if (i == 1) { # If this is out first iteration through the loop, then we don't have a dataframe to add to, however! So in
      ...
      BOT_6 = BOT_5
      futile.logger::flog.info("build_offence_percentages - Created the BOT_6 dataframe")

    } else { # But if it's not our first iteration, then we will just add the column to the dataframe that already exists:

      BOT_6 = base::rbind(BOT_6, BOT_5)
      futile.logger::flog.info("build_offence_percentages - Added BOT_5 to the BOT_6 dataframe")

    }

  }

  # 9.  Calculate percentages by offence type and add to main dataframe
  ...
  Index_row = BOT_6[i, ]
  BOT_7 = as.data.frame(Index_row / All_total * 100)
  futile.logger::flog.info("build_offence_percentages - Calculated the percentages for offence type")

  if (i == 1) { # If this is out first iteration through the loop, then we don't have a dataframe to add to, however! So in
    # that case, we must create one:

    BOT_8 = base::as.data.frame(BOT_7)
    futile.logger::flog.info("build_offence_percentages - Created BOT_8 dataframe")

  } else { # But if it's not our first iteration, then we will just add the column to the dataframe that already exists:

    BOT_8 = base::rbind(BOT_8, BOT_7)
    futile.logger::flog.info("build_offence_percentages - Added BOT_7 to the BOT_8 dataframe")
  ...
  BOT_9 = rbind(BOT_8, Rows_of_unweighted_bases)


  # 11. Name the rows from the config labels

  rownames(BOT_9) = lapply(config$PART_ONE$ROW_LABELS, `[[`, 2)
  futile.logger::flog.info("build_offence_percentages - Renamed the rows for the complete data frame")


  # 12. Return the final dataframe:
  return(BOT_9)
  futile.logger::flog.info("build_offence_percentages - Percentages by Offence Type built")
