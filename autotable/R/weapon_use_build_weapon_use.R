#'@title build_weapon_use
#'
#'@description Builds the "Whether a weapon was used" (WU) part/half of one subtable of an "Use of Weapon"
...
#'
#'@examples build_weapon_use(config = "configs/use_of_weapons/BURGLARY.yaml", master config = master config, subtable = 1)
#'
#'@export

weapon_use_build_weapon_use <- function(config, master_config, subtable) {
...
for (i in c(1:length(Years_of_data))) {
  ...
  futile.logger::flog.info("build_weapon_use - Reading in source data from %s",
  ...
  futile.logger::flog.info("build_weapon_use - Using %s as the weight variable", Weight_variable)
  
  # 5. Remove zero-weighted cases from the dataset:
  WU_1 = autotable::remove_zeroweighted_rows(df = VF_dataset, weightvar = Weight_variable)
  futile.logger::flog.info("build_weapon_use - Removed %s cases out of %s because their weights were 0",
                           nrow(VF_dataset) - nrow(WU_1), nrow(VF_dataset))
  ...
  WU_2 = dplyr::filter(WU_1, victarea == 1 | wherhapp == 1)
  futile.logger::flog.info("build_weapon_use - Selected %s cases where victarea = 1 or wherapp = 1",
                           nrow(WU_2))
  ...
  WU_3 = dplyr::mutate(WU_2, relevant_incident = dplyr::if_else(offence %in% Offence_codes, 1, NULL))
  futile.logger::flog.info("build_weapon_use - Computed a new variable called relevant_incident; a value of 1 for this variable will indicate that the case in question has an offence code relevant to the subtable being built")
  
  # 8. Select only the cases where relevant_incident = 1:
  WU_4 = dplyr::filter(WU_3, relevant_incident == 1)
  futile.logger::flog.info("build_weapon_use - Selected %s cases where relevant_incident = 1",
                           nrow(WU_4))
  
  # 9. Filter out those where vftype == 1:
  WU_5 = dplyr::filter(WU_4, vftype == 1)
  futile.logger::flog.info("build_weapon_use - Selected %s cases where vftype = 1",
                           nrow(WU_5))
  ...
  WU_5 = dplyr::mutate(WU_5, weaponused = dplyr::if_else(weap2 == 1, 1, NULL))
  WU_5 = dplyr::mutate(WU_5, noweaponused = dplyr::if_else(weap2 == 2, 1, NULL))
  WU_5 = dplyr::mutate(WU_5, notknown = dplyr::if_else(weap2 == 3, 1, NULL))
  
  weaponused = filter(WU_5, weaponused == 1)
  noweaponused = filter(WU_5, noweaponused == 1)
  notknown = filter(WU_5, notknown == 1)
  
  WU_5 = filter(WU_5, weaponused == 1 | noweaponused == 1 | notknown == 1)

  # 10. 2 ...and extract the unweighted base:
  
  Unweighted_base = base::sum(WU_5$relevant_incident)
  futile.logger::flog.info("build_weapon_use - Recoded variables and determined the unweighted base as %s",
                           Unweighted_base)
  
  # 11. Weight the data:
  
  Variables_for_rows = unlist(lapply(config$PART_ONE$ROW_LABELS, `[[`, 1))
  WU_6 = autotable::apply_weight(WU_5, base::c("relevant_incident", Variables_for_rows), weight = Weight_variable)
  
  # 12. Generate the unrounded percentages for this year:
  Weighted_totals = dplyr::summarise_at(WU_6, Variables_for_rows, sum, na.rm = TRUE)
  WU_7        = (Weighted_totals / base::sum(WU_6$relevant_incident)) * 100
  futile.logger::flog.info("build_weapon_use - Generated the unrounded percentages")
  ...
  futile.logger::flog.info("build_weapon_use - Finished column \"%s\"", names(Years_of_data)[i])
  ...
  WU_8 = base::as.data.frame(base::t(WU_7))
  Row_of_unweighted_bases = Unweighted_base
  futile.logger::flog.info("build_weapon_use - Added column to the WU_9 dataframe")
    
  } else { # But if it's not our first iteration, then we will just add the column to the dataframe that already exists:
    
  WU_8 = base::cbind(WU_8, base::as.data.frame(base::t(WU_7)))
  Row_of_unweighted_bases = base::c(Row_of_unweighted_bases, Unweighted_base)
  futile.logger::flog.info("build_weapon_use - Added column to the WU_8 dataframe")
  ...
  WU_9 = base::rbind(WU_8, Row_of_unweighted_bases)
  futile.logger::flog.info("build_weapon_use - Added the row of unweighted bases")

  # 16. Name the rows:
  rownames(WU_9) = lapply(config$PART_ONE$ROW_LABELS, `[[`, 2)
  futile.logger::flog.info("build_weapon_use - Named the rows")
  ...
  WU_10 = append_significance_column(dataframe = WU_9, column_1 = ncol(WU_9), column_2 = 1,
                                   row_of_unweighted_bases = nrow(WU_9))
  futile.logger::flog.info("build_weapon_use - Appended a significance column, comparing the latest year of data with the previous year")
  WU_11 = append_significance_column(dataframe = WU_10, column_1 = ncol(WU_10) - 1, column_2 = ncol(WU_10) - 2,
                                   row_of_unweighted_bases = nrow(WU_10))
  futile.logger::flog.info("build_weapon_use - Appended a significance column, comparing the latest year of data with the earliest year")
  ...
  colnames(WU_11) = c(names(Years_of_data),
                       name_of_10_year_test_column,
                       name_of_1_year_test_column)
  
  futile.logger::flog.info("build_weapon_use - Added column labels")
